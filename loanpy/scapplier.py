...
def dfm(mtx):
    """
    Return shortest path as list of tuples using Dijkstra's algorithm.
    This ensures the path with the least edit operations is returned.
    
    :param mtx: The distance matrix generated by ``get_mtx``.
    :type mtx: A list of list of integers
    
    :return: The shortest path through the matrix as a list of tuples
    :rtype: list of tuples
    """
    # Convert the matrix to a weighted graph
    graph = mtx2graph(mtx, w_del=1, w_ins=1)
    ...
    # If no path is found (which shouldn't happen in this context),
    # fall back to the original implementation
    if shortest_path is None:
        shortest_path = [(0, 0)]
        i, j = 0, 0
        while (i, j) != (len(mtx)-1, len(mtx[0])-1):
            try:
                keep = mtx[i][j] == mtx[i+1][j+1]
            except IndexError:
                keep = False
            if keep:
                i += 1
