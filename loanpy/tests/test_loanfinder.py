        ...
        with patch("loanpy.loanfinder.Distance") as Distance_mock:
            ...
            with patch("loanpy.loanfinder.Adrc") as Adrc_mock:
                ...
                for i in msdict:
                    ...
                    if i == "reccol" == "reconstructed": assert msdict[
                    i] == "reconstructed"
                    if i == "recpath": assert msdict[i] == "hun.csv"
                    if i == "semsim": assert msdict[i] == 1
                    if i == "semsim_msr": assert msdict[i] == mock_gensim_mw
    ...
    srsad, srsrc]) as read_data_mock:
        with patch("loanpy.loanfinder.Distance") as Distance_mock:
            ...
            for i in msdict:
                if i in zip(["search_in", "search_for"], [srsad, srsrc]):
                    assert_series_equal(msdict[i], expsrs)
                if i == "doncol": assert msdict[i] == "ad"
                if i == "donpath": assert msdict[i] is None
                if i == "get_nse_ad": assert msdict[i] == AdrcMonkey.get_nse
                if i == "get_nse_rc": assert msdict[i] == AdrcMonkey.get_nse
                if i == "phondist": assert msdict[i] == 0
                if i == "phondist_msr": assert msdict[
                i] == dist_mockinstance.hamming_feature_edit_distance
                if i == "reccol" == "reconstructed": assert msdict[i] == "rc"
                if i == "recpath": assert msdict[i] is None
                if i == "semsim": assert msdict[i] == 1
                #cant mock gensim. If mock plugged in not passed on in arg.
                if i == "semsim_msr": assert msdict[i] == lf.gensim_multiword
...
def test_phonmatch():
    ...
    class_phon = SearchMonkey()
    class_phon.dist = 0.5
    #set up: mock pandas dadta frame, define its return value as var named "df"
    with patch("loanpy.loanfinder.DataFrame") as DataFrame_mock:
        df = DataFrame({"match": ["blub"], "donor_idx": [99]}, index=[1])
        DataFrame_mock.return_value = df
        #assert expected and actual result are identical pandas data frames
    ...
    phon_match_res2, sem_match_res2]) as gen_mock:
        ...
        with patch("loanpy.loanfinder.concat") as concat_mock:
            ...
            dfenrec, dfendon]*2) as read_csv_mock:
                ...
                assert_frame_equal(Search.loans(mocksearch, write_to=path,
                postprocess=True, merge_with_rest=True), exp2)
