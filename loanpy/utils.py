...
class IPA():
    ...
    def __init__(self) -> None:
        ...
        self.back_vowels = [row[ipa[0].index("ipa")] for row in ipa if row[considx] == "-1" and row[ipa[0].index("back")] == "1"]
    ...
    def get_fb(self, ipastr: str) -> str:
        """Input a vowel! Returns 'F' for front vowels and 'B' otherwise."""
        if ipastr is not None and ("a" in ipastr or "ə" in ipastr):
            return "B"  # sorry panphon, a and schwa are a backvowels now.
        return "B" if ipastr in self.back_vowels else "F"
...
def is_alternating_language_sequence(
        data: List[List[str]], source_lang: str, target_lang: str, forms
        ) -> bool:
    ...
    .. code-block:: python
        ...
        >>> from scripts.loanpy import is_valid_language_sequence
        >>> data = [
        ...     ['x', 'x', 'de', 'x', 'x', 'x', 'x', 'x', 'x', '0', 'x'],
        ...     ['x', 'x', 'en', 'x', 'x', 'x', 'x', 'x', 'x', '0', 'x'],
        ...     ['x', 'x', 'de', 'x', 'x', 'x', 'x', 'x', 'x', '1', 'x'],
    ...
    for idx, row in enumerate(data):
        expected_lang = source_lang if idx % 2 == 0 else target_lang
        if forms[row["Form_ID"]]["Language_ID"] not in expected_lang:
            logging.info(f"Problem in row {idx}: {row}")
            return False
    return True
