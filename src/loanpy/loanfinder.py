...
from loanpy.helpers import gensim_multiword, get_clusters
...
nse_combined
        ...
        # read in data for likeliestphonmatch, i.e. col Segments in
        # donor and CV_Segments for recipient data frames
        dfmatches = dfmatches.merge(read_csv(self.recpath, encoding="utf-8",
                                             usecols=["CV_Segments",
                                                      self.reccol]).fillna(""),
                                    left_on="recipdf_idx", right_index=True)
...
club", recip_rc="(b|c)?lub",
        ...
        # get cluster segmentation of matches
        dfph["cv_match"] = [get_clusters(wrd.split(" ")) for wrd in dfph["match"]]
