"""Predict changes in horizontal or vertical transfer of words"""
...
[['d', 't͡ʃ'], ['a', 'e'], ['l'], ['d', 't͡ʃ'], ['e', 'a']], \
  [['d', 't͡ʃ'], ['a', 'e'], ['d', 't͡ʃ'], ['l'], ['e', 'a']]]])])
        ...
        (max_phon, max_repaired_phonotactics,
         max_paths2repaired_phonotactics) = get_howmany(
                                        howmany, max_repaired_phonotactics,
...
# nothing gets filtered
        ...
        if all(i is False or i == 0 for i in
               [phonotactics_filter, vowelharmony_filter]):
        ...
        if phonotactics_filter:
            out = [i for i in out if pros(i) in self.invs["pros"]]
            if out == []:
                return "wrong phonotactics"
        ...
        if howmany != float("inf"):
            out = out[:howmany]
        return f"^{'$|^'.join([sub('[. ]', '', wd) for wd in out])}$"
        # turn list to regular expression
