...
def test_write_to_cache():
    ...
    DataFrame(columns=list(init_args_mock)
    +["optimal_howmany", "opt_tp", "opt_tpr", "timing", "date"]
    ...
    df_exp = DataFrame(
        ...
        "max_phonotactics": [1]*2, "max_paths": [1]*2, "writesc": [False]*2,
...
def test_cache():
    ...
    try: remove(mockpath2cache)  # in case of leftovers from previous tests
    except FileNotFoundError: pass
...
def test_eval_adapt():
    ...
    10, False, False, False, False, 0, 1, 100, 49, False) == {
    ...
    # assert target hit while showing workflow, no adapt_phonotactics
    assert eval_adapt("dat͡ʃa", adrc_obj, "dat͡ʃːa",
    10, False, False, False, False, 0, 1, 100, 49, True) == {
    'best_guess': 'dat͡ʃa', 'guesses': 1,
    'adapted_phonotactics': "[['d', 'a', 't͡ʃː', 'a']]",
...
def test_eval_one():
    ...
    assert eval_one("gaga", adrc_obj, "dada", False,
    False, False, False, 0, 1, 100, 49, False, [2, 4, 6], "adapt") == {
    "guesses": float("inf"), "best_guess": "dada"}
    #assert no keyerror, mode=adapt, target missed
    assert eval_one("gaga", adrc_obj, "dada", False,
    False, False, False, False, 0, 1, 100, 49, [2, 4, 6], "adapt") == {
    "guesses": float("inf"), "best_guess": "dada"}
    #assert target hit on first try, mode=adapt
    assert eval_one("dada", adrc_obj, "dada", False,
    False, False, False, 0, 1, 100, 49, False, [1], "adapt") == {
    "guesses": 1, "best_guess": "dada"}
    ...
    assert eval_one("gaga", adrc_obj, "dada", False,
    False, False, False, 0, 1, 100, 49, False, [2, 4, 6], "reconstruct") == {
    "guesses": float("inf"), "best_guess": '#d, a, d, a# not old'}
    #assert no keyerror, mode=reconstruct, target missed
    assert eval_one("gaga", adrc_obj, "aːruː", False,
    False, False, False, False, 0, 1, 100, 49, [2, 4, 6], "reconstruct") == {
    "guesses": float("inf"), "best_guess": "^(a)(n)(a)(at͡ʃi|ɣ)$"}
    #assert target hit on first try, mode=reconstruct
    assert eval_one("anaat͡ʃi", adrc_obj, "aːruː", False,
    False, False, False, 0, 1, 100, 49, False, [1], "reconstruct") == {
    "guesses": 1, "best_guess": "^(a)(n)(a)(at͡ʃi)$"}
...
def test_get_noncrossval_sc():
    ...
    ['VCCVC', 'VCVC', 'VCVCV'], ['VCVC', 'VCCVC', 'VCVCV']]):
        assert set(adrc_obj_out.scdict_phonotactics[struc]) == set(exp_phonotactics)
    ...
    adrc_obj_out = get_noncrossval_sc(adrc_obj, None)
    #assert result
    assert adrc_obj_out.scdict == {'#-': ['-'], '#a': ['aː', 'ɒ'],
    '-#': ['oz'], 'a': ['uː', 'aː'], 'at͡ʃi#': ['-'], 'j': ['jn'],
    'ld': ['ɟ'], 'n#': ['r'], 'ɣ': ['t͡ʃ'], 'ɣ#': ['-']}
    assert adrc_obj_out.sedict == {'#-<*-': 3, '#a<*aː': 2, '#a<*ɒ': 1,
    '-#<*oz': 1, 'a<*aː': 1, 'a<*uː': 1, 'at͡ʃi#<*-': 1, 'j<*jn': 1,
    'ld<*ɟ': 1, 'n#<*r': 1, 'ɣ#<*-': 1, 'ɣ<*t͡ʃ': 1}
    ...
    't͡ʃ<t͡ʃː': 1, 'ɣ<ɣ': 2},
    {'a<a': [1, 2, 3], 'd<d': [2], 'i<ɯ': [1], 'j<j': [3], 'l<l': [2],
...
def test_get_crossval_data():
    ...
    #test with mode="reconstruct"
    #set up
    adrc_obj = Adrc(forms_csv=PATH2FORMS,
    source_language="H", target_language="EAH", mode="reconstruct")
    #run function
    #first cog isolated, missing sc: a j a n - a j a n
    adrc_obj_out = get_crossval_data(adrc_obj, 2, None)
    #assert result
    assert adrc_obj_out.scdict == {'#-': ['-'], '#a': ['aː'], 'a': ['uː'],
    'at͡ʃi#': ['-'], 'ld': ['ɟ'], 'ɣ': ['t͡ʃ'], 'ɣ#': ['-']}
    assert adrc_obj_out.sedict == {'#-<*-': 2, '#a<*aː': 2, 'a<*uː': 1,
    'at͡ʃi#<*-': 1, 'ld<*ɟ': 1, 'ɣ#<*-': 1, 'ɣ<*t͡ʃ': 1}
    assert adrc_obj_out.scdict_phonotactics == {}

    #test with writesc = Path()
    #set up
    path2outfolder = Path(__file__).parent / "output_files"  # has to be folder!
    adrc_obj = Adrc(forms_csv=PATH2FORMS,
    source_language="H", target_language="EAH", mode="reconstruct")
    #run function
    #first cog isolated, missing sc: a j a n - a j a n
    ...
    {'#-': ['-'], '#a': ['aː'], 'a': ['uː'], 'at͡ʃi#': ['-'],
    'ld': ['ɟ'], 'ɣ': ['t͡ʃ'], 'ɣ#': ['-']}, {'#-<*-': 2, '#a<*aː': 2,
    'a<*uː': 1, 'at͡ʃi#<*-': 1, 'ld<*ɟ': 1, 'ɣ#<*-': 1, 'ɣ<*t͡ʃ': 1},
...
def test_loop_thru_data():
    ...
    assert loop_thru_data(adrc_obj, 1, 1, 100, 49,
    ...
    columns=['Target_Form', 'Source_Form', 'Cognacy', 'guesses', 'best_guess'])
...
def test_getnse4df():
    ...
    #test with mode="reconstruct"
    adrc_obj = Adrc(forms_csv=PATH2FORMS, source_language="H",
    target_language="EAH", scdictlist=PATH2SC_RC, mode="reconstruct")
...
def test_postprocess():
    ...
    adrc_obj_out = postprocess(adrc_obj)
    assert_frame_equal(adrc_obj_out.dfety, read_csv(
    Path(__file__).parent / "expected_files" / "postprocess_ad_workflow.csv"))
